...
export default class ChatWelcome {
  ...
  setup() {
    ...
    const has_socials = instagram || tiktok || facebook || tg_channel;
    ...
    if (has_channels) {
      let btns = '';
      if (whatsapp) {
        const wa_link = `https://wa.me/${whatsapp.replace(/[^0-9+]/g, '')}`;
        btns += `<a href="${wa_link}" target="_blank" rel="noopener" class="chat-channel-btn whatsapp" title="WhatsApp">
      ...
      if (telegram) {
        btns += `<a href="${telegram}" target="_blank" rel="noopener" class="chat-channel-btn telegram" title="Telegram">
          <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0h-.056zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
          <span>Telegram</span>
        </a>`;
      }
      if (phone) {
        btns += `<a href="tel:${phone}" class="chat-channel-btn phone" title="${__('Call Us')}">
          <svg viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>
          <span>${__('Call')}</span>
        </a>`;
      }
      channels_html = `
        <div class='chat-channels-divider'>
          <span>${__('Or reach us directly')}</span>
        </div>
    ...
    // Save contact button
    let save_contact_html = '';
    if (enable_save) {
      save_contact_html = `
        <a href="/api/method/chat.api.contact.download_vcard" class="chat-save-contact-btn" title="${__('Save our contact to your phone')}">
          <svg viewBox="0 0 24 24" width="15" height="15"><path fill="currentColor" d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10h-4v4h-2v-4H7v-2h4V7h2v4h4v2z"/></svg>
    ...
    if (has_socials) {
      let icons = '';
      if (instagram) {
        icons += `<a href="${instagram}" target="_blank" rel="noopener" class="chat-social-icon" title="Instagram">
      ...
      if (tiktok) {
        icons += `<a href="${tiktok}" target="_blank" rel="noopener" class="chat-social-icon" title="TikTok">
          <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>
        </a>`;
      }
      if (facebook) {
        icons += `<a href="${facebook}" target="_blank" rel="noopener" class="chat-social-icon" title="Facebook">
          <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </a>`;
      }
      if (tg_channel) {
        icons += `<a href="${tg_channel}" target="_blank" rel="noopener" class="chat-social-icon" title="Telegram Channel">
          <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0h-.056zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
        </a>`;
      }
      socials_html = `<div class='chat-social-icons'>${icons}</div>`;
    }

    const html = `
      <div class='chat-welcome-header'>
        <div class='chat-welcome-header-left'>
          ${logo_html}
          <span class='chat-welcome-brand-name'>${__(brand.name || '')}</span>
        </div>
        ${show_badge ? `<div class='chat-247-badge'><span class='chat-247-dot'></span>${__(badge_text)}</div>` : ''}
      </div>
